              ...
              <tr key={member.id}>
                ...
                <AdminTd>
                  <div className="flex flex-wrap gap-2">
                    <Link className="rounded-md border border-slate-200 px-3 py-2 text-xs font-semibold transition hover:bg-emerald-50 hover:text-emerald-800" href={`/admin/members/${member.id}/edit`}>Edit</Link>
                    <form action={deleteMemberAction.bind(null, member.id)}><button className="rounded-md border border-red-200 px-3 py-2 text-xs font-semibold text-red-800">Delete</button></form>
                  </div>
                </AdminTd>
              </tr>
            ))}
